...
parse([$(|Rest]) ->                            % starts with a '('
      ...
      [Op|Rest2]     = Rest1,                  % then an operator, '+' or '*' or '-'
...
run([{sub2} | Continue], Env ,[N1,N2|Stack]) ->
    run(Continue, Env, [(N1-N2) | Stack]);
run([{abs2} | Continue], Env ,[N1|Stack]) ->
    run(Continue, Env, [abs(N1) | Stack]);
run([],_Env,[N]) ->
    N.
...
simplify({sub,E1,E2}) ->
    simplify({sub,simplify(E1),simplify(E2)});
simplify(E) ->
    E.
...
% list of keys from the env
getEnvKeys([]) -> [];
getEnvKeys([N]) ->
    [N1|_] = string:tokens(N, "="),
    [list_to_atom(N1)];
getEnvKeys([N|Env]) ->
    [N1|_] = string:tokens(N, "="),
    lists:merge([list_to_atom(N1)], getEnvKeys(Env)).
...
isInList([Var|_], V) when V == Var -> true;
isInList([_|L], V) -> isInList(L, V).
...
env_check(_, {num, _}) ->
    true;
env_check(Env, {var, V}) ->
    isInList(getEnvKeys(Env), V);
env_check(Env, {_, {var, V}, E2}) ->
    isInList(getEnvKeys(Env), V) and env_check(Env, E2);
env_check(Env, {_, E1, {var, V}}) ->
    isInList(getEnvKeys(Env), V) and env_check(Env, E1);
env_check(Env, {abs, {var, V}}) ->
    isInList(getEnvKeys(Env), V);
env_check(Env, {_, E1, E2}) ->
    env_check(Env, E1) and env_check(Env, E2).
